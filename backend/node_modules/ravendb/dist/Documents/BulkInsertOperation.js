...
class BulkInsertOperation {
    constructor(database, store, options) {
        ...
        this._bulkInsertExecuteTaskErrored = false;
        this._first = true;
        ...
        this._requestBodyStreamFinished = false;
        this._asyncWrite = Promise.resolve();
    ...
    async sendHeartBeat() {
        ...
        const context = (0, SemaphoreUtil_1.acquireSemaphore)(this._streamLock, {
            timeout: 0
        });
        try {
    ...
    async _writeToStream(entity, id, metadata, type) {
        try {
            if (this._first) {
                this._first = false;
            }
    ...
    _handleErrors(documentId, e) {
        ...
        (0, Exceptions_1.throwError)("InvalidOperationException", "Bulk insert error, Document Id: " + documentId, e);
    }
    ...
    async writeToRequestBodyStream(buffer, force = false) {
        try {
            ...
            if (this._isInitialWrite || force) {
                ...
                if (this._compressedStream) {
                    const flush = (0, node_util_1.promisify)(this._compressedStream.flush);
    ...
    async _ensureStream() {
        try {
            ...
            if (this.useCompression) {
                this._compressedStream = (0, node_zlib_1.createGzip)();
                (0, readable_stream_1.pipeline)(this._requestBodyStream, this._compressedStream);
            }
            const bulkCommand = new BulkInsertCommand(this._operationId, this._compressedStream ?? this._requestBodyStream, this._nodeTag, this._options.skipOverwriteIfUnchanged);
    ...
    async abort() {
        if (this._operationId !== -1) {
            ...
            try {
                ...
                (0, Exceptions_1.throwError)("BulkInsertAbortedException", "Unable to kill bulk insert operation, because it was not found on the server.", err);
            }
        }
    }
    async finish() {
...
BulkInsertOperation._maxSizeInBuffer = 1024 * 1024;
