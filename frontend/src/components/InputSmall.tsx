...
export interface InputSmallProps
...
const InputSmall = React.forwardRef<HTMLInputElement, InputSmallProps>(
      ...
      <div className="w-full">
        <div className="relative my-4 floating-label-container">
          <input
            type={type}
            className={cn(
              'h-10 w-full rounded-sm border-2 pl-2.5 pr-3 pt-4 pb-2 text-sm text-portola-green bg-cloud font-serif focus:outline-none focus:ring-0 focus:ring-offset-0 focus:shadow-none disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-sand floating-input',
          ...
          {label && (
            <label className={cn(
              'pointer-events-none absolute left-3 font-serif transition-all duration-200 floating-label',
              isFocused || hasValue 
                ? 'top-0.5 text-xs text-grass' 
                : showError && error 
