...
export interface InputLargeProps
      ...
      <div className="w-full">
        <div className="relative my-4 floating-label-container">
          <input
            type={type}
            className={cn(
              'h-14 w-full rounded-sm border-2 pl-5 pr-5 pt-6 pb-2 text-lg text-portola-green bg-cloud font-serif focus:outline-none focus:ring-0 focus:ring-offset-0 focus:shadow-none disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-sand floating-input',
          ...
          {label && (
            <label className={cn(
              'pointer-events-none absolute left-5 font-serif transition-all duration-200 floating-label',
              isFocused || hasValue 
                ? 'top-1 text-sm text-grass' 
                : showError && error 
                  ? 'top-1/2 -translate-y-1/2 text-lg text-alert'
                  : 'top-1/2 -translate-y-1/2 text-lg text-steel'
            )}>
              {label}
            </label>
          )}
